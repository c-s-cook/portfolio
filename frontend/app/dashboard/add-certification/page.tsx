...
import type { Certification, Portfolio } from '../../../lib/types';
...
export default function AddProject() {
  ...
  let today = new Date();
  let dateString = String(today.getFullYear()) + "-" + String(today.getMonth() + 1) + "-" + String(today.getDate());
  let certCount = useRef(0);
  let portfolio = useRef<Portfolio>({} as Portfolio);
  let certSlugs = useRef([]);
  ...
  const getCertCount = async () => {
    ...
    if (portfolio.current && !portfolio.current.error) {
      // set the certCount & certification ID...
      certCount.current = portfolio.current.certifications!.length;
      newCert.id = certCount.current || 0;

      // load existing certification URL slugs for safetfy checking...
      certSlugs.current = portfolio.current.projects.map(proj => proj.slug).filter(slug => slug !== undefined);
      console.log('certSlugs.current: ', certSlugs.current);

      console.log(`aquired portfolio. setting new .renameTo...${String(certCount.current).padStart(2, '0')}_${dateString}`);
  ...
  let photoUploadProps: PhotoUploadProps = {
    ...
    options: {
      ...
      autoUpload: {
        ...
        renameTo: certCount.current ? `${String(certCount.current).padStart(2, '0')}_${dateString}` : dateString,
        maxRetries: 3,
        delay: 60,
      }
    }
  ...
  useEffect(() => {
    ...
    // check for auto-saved tempCert...
    let tempCertString: string | Certification = localStorage.getItem('tempCert')
    if (tempCertString) {
      console.log('found auto-save...');
      canAutoSave.current = false;

      const loadAutoSave = async () => {
        let tempCert = await JSON.parse(tempCertString) as Certification;

        setTimeout(() => {

          setTitle(tempCert.title);
          setSlug(tempCert.slug);
          quillRef.current.root.innerHTML = tempCert.body;
          setTags(tempCert.tags);
          setImageURLs(tempCert.thumbnails);
          setRepoUrl(tempCert.repoUrl);
          setCertUrl(tempCert.certUrl);
          setDate(tempCert.date);
  ...
  useEffect(() => {
    ...
    if (certSlugs.current && certSlugs.current.includes(tempSlug)) {
  ...
  useEffect(() => {
    ...
    if (canAutoSave.current && typeof (Storage) !== "undefined") {
      ...
      setTimeout(() => {
        localStorage.setItem('tempCert', JSON.stringify(newCert));
  ...
  }, [title, slug, content, tags, imageURLs, repoUrl, certUrl])
  ...
  const validateForm = async (e?) => {
    ...
    const postNewCert = async () => {
      // send the newCert object to the 'api/portfolio' api with a POST call in a try/catch block
      try {
    ...
    // validate all the newCert fields / properties...
    ...
    else if (certSlugs.current && certSlugs.current.includes(slug)) {
    ...
    if (tags.length === 0) {
      tagsError.innerText = 'Missing certification tags.';
    ...
    //  check live site URL...
    let liveSiteError = document.querySelector('.item-certUrl .error-msg') as HTMLSpanElement;
    if (!certUrl && publishingChecks.current.certUrl !== 'proceed') {
      liveSiteError.innerText = 'Should there be a link to a live site?';
      publishingChecks.current['certUrl'] = 'flagged';
      ...
      <section>
        <div className="content with-background add-item extra-long">
          ...
          <form id="add-item-form">
            ...
            {/* add a date input selector that offers a calendar pop-up selector */}
            <div className="item-date">
              <label htmlFor="date">Date: <span className="error-msg"></span></label>
              <input
                type='date'
                id='date'
            ...
            <div className="item-certUrl">
              <label htmlFor="certUrl">Link to Certification: <span className="error-msg"></span></label>
              <input
                type='text'
                id='certUrl'
                value={certUrl}
