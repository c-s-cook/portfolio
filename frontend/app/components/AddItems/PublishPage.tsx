...
// function ForDynamicPublishPortfolioItem() {
  ...
  const params = useParams()?.params || null;
  ...
  const loadAutoSave = async () => {
    ...
    let localItem: string | null = typeof window !== 'undefined' ? await sessionStorage.getItem(`autoSaved${itemType.toUpperCase()}`) : null;
    if (!localItem) return
  ...
  const autoSave = async () => {
    // safety check: is there a DOM? Then why do bad pings happen?
  ...
  const clearForm = async () => {
    ...
    let localItem: string | null = typeof window !== 'undefined' ? sessionStorage.getItem(`autoSaved${itemType.toUpperCase()}`) : null;
    if (localItem) {
 ...
 */
  ...
  useEffect(() => {
    ...
    if (itemType === 'project' || itemType === 'certification') {
      (async () => {
        ...
        // check for auto-saved item...
        // within a useEffect, so it's safe to assume the DOM is loaded (...right?)
        let localItem: string | null = sessionStorage.getItem(`autoSaved${itemType.toUpperCase()}`) || null;
        if (localItem) {
          console.log('found auto-save...');
          // canAutoSave.current = false;
          if (checkNonEmptyItem(await JSON.parse(localItem))) setCanLoadAutoSave(true);
  ...
  const validateForm = async (e?) => {
    ...
    let pubBtn = document.getElementById('publish') as HTMLButtonElement || null;
    if (pubBtn) {
